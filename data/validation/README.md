...
| quantitative_result_type | quantitative_result の根拠種別 | 下記「quantitative_result_type の判定基準」から1つを選択する。 |
...
「続報未確認」は、最終確認日（last_checked）時点で公開情報から続報を確認できていない
状態を指す。事業が終了したこと、または続報が存在しないことを意味するものではない。

## quantitative_result_type の判定基準

quantitative_result に記載した数値成果が、どのような根拠に基づくかを次の6種類から1つ選ぶ。

| 値 | 判定基準 |
|---|---|
| 実測 | 実際の利用結果、アンケート、作業時間の計測など、実績として確認された結果に基づく。 |
| 実測からの推計 | 一部職員・一部期間などの実績値を、全庁・年間規模などへ換算した値。 |
| 事前試算 | 実証前または本導入前に、想定・試算として計算された削減効果。 |
| 目標 | 将来達成する予定として設定された数値（実績ではない）。 |
| 定性評価 | 「利用可能」との評価、満足度、継続利用意向など、数値の実測を伴わない定性的な評価が中心の結果。 |
| unknown | quantitative_result の根拠種別を公開情報から判断できない場合。 |

- 実績（実測・実測からの推計）と、目標・事前試算は明確に区別する。実績ではない数値を
  実績と混同して「実測」「実測からの推計」を選ばない。
- 導入人数、対象人数、PC台数、参加団体数などの規模・利用者数の情報のみでは、
  業務効果（時間削減・満足度向上等）を示す quantitative_result / quantitative_result_type
  の根拠として扱わない。業務効果を示す数値が別途確認できない場合、
  quantitative_result_type は unknown とする。
